...
std::string TemplateNDK::start(const std::string& arg) {
	...
	bool parse = reader.parse(arg, root);
 	...
 	if (parse) {
 		aGameId = strdup(root["slGameId"].asCString());
 		aGameSecret = strdup(root["slGameSecret"].asCString());
 		aGameVersion = strdup(root["slGameVersion"].asCString());
 		aCurrency = strdup(root["slCurrency"].asCString());
 		aLanguageCode = strdup(root["slLanguageCode"].asCString());
 	}
	...
	root["aGameId"] = aGameId;
	root["aGameSecret"] = aGameSecret;
	root["aGameVersion"] = aGameVersion;
	root["aCurrency"] = aCurrency;
	root["aLanguageCode"] = aLanguageCode;
...
void TemplateNDK::freebuddies(AppData_t app) {
...
SC_Error_t TemplateNDK::scsetscore(AppData_t *app, double aScore, double *aMinorScore = NULL, unsigned int *aLevel = NULL, unsigned int *aMode = NULL) {
	SC_Error_t rc;
...
SC_Bool_t TemplateNDK::scscoreshasnextrange(AppData_t *app) {
	SC_Bool_t  rc;
...
SC_Error_t TemplateNDK::scscoresgetnextrange(AppData_t *app) {
	SC_Error_t  rc;
...
SC_Bool_t TemplateNDK::scscoreshasprevrange(AppData_t *app) {
	SC_Bool_t  rc;
...
SC_Error_t TemplateNDK::scscoresgetprevrange(AppData_t *app) {
	SC_Error_t  rc;
...
SC_Error_t TemplateNDK::scscoresfree(AppData_t *app) {
	SC_Error_t rc;
...
SC_Error_t TemplateNDK::scgetscores(AppData_t *app, unsigned int sMode, const SC_ScoresSearchList_t searchList, unsigned int rangeLength) {
	SC_Error_t rc;
	//create user controller
	if(app->scoresController != NULL) {
...
SC_Bool_t TemplateNDK::scgameshasnextrange(AppData_t *app) {
	SC_Bool_t  rc;
...
SC_Error_t TemplateNDK::scgamesgetnextrange(AppData_t *app) {
	SC_Error_t  rc;
...
SC_Bool_t TemplateNDK::scgameshasprevrange(AppData_t *app) {
	SC_Bool_t  rc;
...
SC_Error_t TemplateNDK::scgamesgetprevrange(AppData_t *app) {
	SC_Error_t  rc;
...
SC_Error_t TemplateNDK::scgetgames(AppData_t *app, SC_GamesSearchList_t filter, unsigned int rangeLength = 0) {
	SC_Error_t rc;
	SC_Range_t range;
...
SC_Error_t TemplateNDK::sccreatemoney(AppData_t *app, unsigned int amount) {
	SC_Error_t rc;
	unsigned long cash;
...
SC_Error_t TemplateNDK::sccreatechallenge(AppData_t *app, unsigned int amount, unsigned int mode, unsigned int level, SC_User_h against = NULL) {
	SC_Error_t rc;
...
SC_Error_t TemplateNDK::scgetchallengelist(AppData_t *app) {
	SC_Error_t rc;
